...
namespace ConstructiveSolidGeometry
{
    ...
    public class CSG
    {
        ...
        public Mesh toMesh()
        {
			...
			Color[] cols = new Color[this.polygons.Count * 3];
            Mesh m = new Mesh();
            for (int i = 0; i < vertices.Count; i++)
            {
				...
				uvs[i] = vertices[i].uv;
				cols[i] = new Color(vertices[i].zone,0,0);
            }
            m.vertices = verts;
            m.normals = normals;
			m.colors = cols;
		...
		public static CSG fromMeshNT(Mesh m)
		{
			List<Polygon> triangles = new List<Polygon>();
			int[] tris = m.triangles;
			...
			{
				Vertex[] vs = new Vertex[3];
				vs[0] = TranslateVertexNT(m, tris[t]);
				vs[1] = TranslateVertexNT(m, tris[t + 1]);
				vs[2] = TranslateVertexNT(m, tris[t + 2]);
				//Debug.Log("Tri index: " + (t+i).ToString() + ", Vertex: " + vs[i].pos);
				triangles.Add(new Polygon(vs));
			}
			//Debug.Log("Poly " + triangles.Count);
		...
		private static Vertex TranslateVertexNT(Mesh m, int tri)
		{
			Vector3 pos = (m.vertices[tri]);
			...
			return new Vertex(pos, norm, uv, zone);
		}
