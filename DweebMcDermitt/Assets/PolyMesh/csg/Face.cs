...
public class Face : System.ICloneable {
	...
	/// In faces.
	/// </param>
	static public  List<Face> MergeCoplanars( List<Face> inFaces )
	{
		// 
		for( int i = 0; i < inFaces.Count; ++i )
		{
			for( int j = 0; j < inFaces.Count; ++j )
			{
				if (j == i)
					continue;
				if (inFaces[i] == null || inFaces[j] == null)
					continue;
				// if we are sharing the same side
				if( Vector3.Dot( inFaces[i].GetPlane().normal, inFaces[j].GetPlane().normal ) > 0.995f)
				{
					Face newFace;
