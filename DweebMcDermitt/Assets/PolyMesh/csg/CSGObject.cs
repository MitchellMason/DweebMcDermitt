...
public class CSGObject : MonoBehaviour {
	...
	public void PerformCSG( List<CsgOperation.ECsgOperation> addModes, GameObject[] inSlaves )
	{
		// 
		if (faces == null)
			faces = new List<Face>();
		faces.Clear ();
		BspGen gen = new BspGen( 0 );
		//BspHelper.FacesFromNodes( rootNode, faces );
		...
		foreach ( GameObject g in inSlaves )
		{
			...
			{	
				...
				if (!init)
				{
					if (addModes[count] != CsgOperation.ECsgOperation.CsgOper_Additive)
						continue;
					faces = slave.faces;
					//gen = genSlave;
					rootNode = slave.rootNode;
					init = true;
					count++;
					continue;
				}
				...
				// save faces
				if( addModes[count] == CsgOperation.ECsgOperation.CsgOper_Additive ||
				   addModes[count] == CsgOperation.ECsgOperation.CsgOper_Subtractive )
				{
					faces.Clear();
					CsgOperation oper = new CsgOperation( visitor );
					oper.Perform( addModes[count], this, slave );
					BspHelper.FacesFromNodes( rootNode, faces );
					rootNode = gen.GenerateBspTree(faces);
		...
		//faces = savedFaces;
		faces = Face.MergeCoplanars( faces );
	...
	public void PerformCSGDef( List<CsgOperation.ECsgOperation> addModes, GameObject[] inSlaves )
	{
		if (faces != null)
			faces.Clear ();
		else
		...
		foreach ( GameObject g in inSlaves )
		{
			...
			// if we have a csg object and we are not our self
			// and intersecting
			if(  (slave && intersect(slave) ))
			{	
				if (slave.faces != null)
				slave.faces.Clear();
				else
					slave.faces = new List<Face>();
				//Debug.Log(g.name);
				slave.CreateFromMesh();
