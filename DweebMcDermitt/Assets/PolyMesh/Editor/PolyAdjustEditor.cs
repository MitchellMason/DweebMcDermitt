...
{
	[CustomEditor(typeof(PolyAdjust)), CanEditMultipleObjects]
	public class PolyAdjustEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			if (!multiple)
			{
				...
				if (polyAdjust.keyPoints.Count == 0)
					CreateSquare(polyAdjust, 0.5f);
				...
				else if (GUILayout.Button("Edit PolyAdjust"))
				{
					editing = true;
				...
				{
					polyAdjust.height = EditorGUILayout.FloatField("Height", polyAdjust.height);
					polyAdjust.floor = EditorGUILayout.FloatField("Floor", polyAdjust.floor);
					polyAdjust.addMode = EditorGUILayout.IntField("Addmode", polyAdjust.addMode);
					polyAdjust.zIndex = EditorGUILayout.FloatField("Z-Index", polyAdjust.zIndex);
					
					if (polyAdjust.ShaderToUse == null)
					{
						polyAdjust.ShaderToUse = Shader.Find("Bumped Diffuse");
						Material mat = new Material(polyAdjust.ShaderToUse);
						polyAdjust.textureNames = MeshUtils.getTextures(mat);
						polyAdjust.texturesToUse = new List<Texture2D>();
						for (int i = 0; i < polyAdjust.textureNames.Count; ++i)
						{
							polyAdjust.texturesToUse.Add(new Texture2D(1,1));
							polyAdjust.texturesToUse[i].name = "Click to set " + polyAdjust.textureNames[i];
							//buttontext.Add ("Select a texture and click here to set " + options.textureNames[i]);
						}
					}
					Shader last = EditorGUILayout.ObjectField(polyAdjust.ShaderToUse, typeof(Shader), true) as Shader;
					
					if (polyAdjust.ShaderToUse != last || !ran)
					{
						ran = true;
						if (polyAdjust.ShaderToUse != last)
						{
							polyAdjust.ShaderToUse = last;
							Material mat = new Material(polyAdjust.ShaderToUse);
					...
					for (int i = 0; i < polyAdjust.textureNames.Count; ++i)
					{
						polyAdjust.texturesToUse[i] = EditorGUILayout.ObjectField(polyAdjust.texturesToUse[i], typeof(Texture2D), true) as Texture2D;
					}
					EditorGUILayout.Space();
					if (GUILayout.Button("Build Mesh"))
					{
						polyAdjust.BuildFinishedMesh();
					}
				}
				//Editor settings
		...
		void OnSceneGUI()
		{
			...
			if (!multiple)
			{
				...
				if (keyPoints == null)
				{
					keyPoints = new List<Vector3>(polyAdjust.keyPoints);	
				}
				
				//Load handle matrix
				Handles.matrix = polyAdjust.transform.localToWorldMatrix;
				...
				if (editing)
				{
					...
					worldToLocal = polyAdjust.transform.worldToLocalMatrix;
					inverseRotation = Quaternion.Inverse(polyAdjust.transform.rotation) * Camera.current.transform.rotation;
					snap = gridSnap;
					
					//Update mouse position
					screenMousePosition = new Vector3(e.mousePosition.x, Camera.current.pixelHeight - e.mousePosition.y);
					var plane = new Plane(-polyAdjust.transform.forward, polyAdjust.transform.position);
		...
		{
			if (polyAdjust.GetComponent<Renderer>() != null)
				EditorUtility.SetSelectedWireframeHidden(polyAdjust.GetComponent<Renderer>(), hide);
		}

		void RecordUndo()
		{
			Undo.RecordObject(polyAdjust, "PolyMesh Changed");
		}

		void RecordDeepUndo()
		{
			Undo.RegisterFullObjectHierarchyUndo(polyAdjust, "PolyMesh Changed");
		...
		void TransformPoly(Matrix4x4 matrix)
		{
			for (int i = 0; i < keyPoints.Count; i++)
			{
				keyPoints[i] = matrix.MultiplyPoint(polyAdjust.keyPoints[i]);
			}
		}
		...
		void UpdatePoly(bool sizeChanged, bool recordUndo)
		{
			...
			if (sizeChanged)
			{
				polyAdjust.keyPoints = new List<Vector3>(keyPoints);
			}
			else
			{
				for (int i = 0; i < keyPoints.Count; i++)
				{
					polyAdjust.keyPoints[i] = keyPoints[i];
				}
			}
			polyAdjust.BuildFinishedMesh();
		}
		...
		void MoveKeyPoint(int index, Vector3 amount)
		{
			...
			if (doSnap)
			{
				Vector3 rnd = polyAdjust.transform.TransformPoint(new Vector3(0,0,0));
				rnd = rnd-Snap (rnd);
				keyPoints[index] = Snap(polyAdjust.keyPoints[index] + amount)+rnd;
				
			}
			else
			{
				keyPoints[index] = polyAdjust.keyPoints[index] + amount;
			}
		}
		...
		{
			if (polyAdjust.keyPoints.Count > 1)
			{
				var center = Vector3.zero;
				var area = 0f;
				var b = polyAdjust.keyPoints[polyAdjust.keyPoints.Count - 1];
				foreach (var i in selectedIndices)
				{
					var a = polyAdjust.keyPoints[i];
		...
		PolyAdjust polyAdjust
		{
			get { return (PolyAdjust)target; }
		}
		...
		static void CreateSquare(PolyAdjust polyAdjust, float size)
		{
			polyAdjust.keyPoints.AddRange(new Vector3[] { new Vector3(size, size), new Vector3(size, -size), new Vector3(-size, -size), new Vector3(-size, size)} );
